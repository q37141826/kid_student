...
public class CirclePageIndicator extends View implements PageIndicator {
    ...
    public CirclePageIndicator(Context context, AttributeSet attrs, int defStyle) {
        ...
        // 控制 是否居中显示 默认居中
        mCentered = a
                .getBoolean(R.styleable.CirclePageIndicator_centered, true);
        // 控制方向。 横排还是竖排 0 为横向，1为竖向
        mOrientation = a.getInt(
                R.styleable.CirclePageIndicator_android_orientation, 0);
        mPaintStroke.setStyle(Style.STROKE);
        // 默认图标样式
        mPaintStroke.setColor(a.getColor(
                R.styleable.CirclePageIndicator_strokeColor, getResources().getColor(R.color.gray)));
        // 默认图标 画笔宽度
        mPaintStroke.setStrokeWidth(a.getDimension(
                R.styleable.CirclePageIndicator_strokeWidth, 2));
        mPaintFill.setStyle(Style.FILL);
        // 当前选中图标的颜色
        mPaintFill.setColor(a.getColor(
                R.styleable.CirclePageIndicator_fillColor, getResources().getColor(R.color.app_bg)));
        // 选择图标的半径
        mRadius = a.getDimension(R.styleable.CirclePageIndicator_radius, 3);
        // 是否提前显示
        mSnap = a.getBoolean(R.styleable.CirclePageIndicator_snap, false);
        // 图标与图标之间的间距
        mSpacing = a.getDimension(R.styleable.CirclePageIndicator_spacing, 10);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float cx = (mSnap ? mSnapPage : mCurrentPage) * threeRadius
                + (mSnap ? mSnapPage : mCurrentPage) * mSpacing;
        if (!mSnap) {
            cx += mPageOffset * threeRadius + mPageOffset * mSpacing;
        }
        if (mOrientation == HORIZONTAL) {
            dX = longOffset + cx;
            dY = shortOffset;

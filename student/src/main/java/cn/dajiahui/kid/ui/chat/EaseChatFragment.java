...
public class EaseChatFragment extends EaseBaseFragment {
    ...
    protected View initinitLayout(LayoutInflater inflater) {
        return inflater.inflate(R.layout.ease_fragment_chat, null);
    }
    ...
    public void onViewCreated(View view, Bundle savedInstanceState) {
        ...
        // 按住说话录音控件
        voiceRecorderView = getView(R.id.voice_recorder);
        ...
        inputMenu = getView(R.id.input_menu);
    ...
    protected void setUpView() {
        setfxTtitle(toChatUsername);
        onBackText();
        if (chatType == EaseConstant.CHATTYPE_SINGLE) { // 单聊
            // 设置标题
            if (EaseUserUtils.getUserInfo(toChatUsername) != null) {
                setfxTtitle(EaseUserUtils.getUserInfo(toChatUsername).getNick());
            }
            onRightBtn(R.drawable.em_mm_title_remove, R.string.ease_null);
        } else {
            onRightBtn(R.drawable.em_to_group_details_normal, R.string.ease_null);
            if (chatType == EaseConstant.CHATTYPE_GROUP) {
                // 群聊
                EMGroup group = EMClient.getInstance().groupManager().getGroup(toChatUsername);
                if (group != null)
                    setfxTtitle(group.getGroupName());
                // 监听当前会话的群聊解散被T事件
        ...
        if (resultCode == Activity.RESULT_OK) {
            if (requestCode == REQUEST_CODE_CAMERA) { // 发送照片
                ...
                if (locationAddress != null && !locationAddress.equals("")) {
                    ...
                    ToastUtil.showToast(getActivity(), R.string.unable_to_get_loaction);
                }
...
//        }
            ...
            switch (itemId) {
                case ITEM_TAKE_PICTURE: // 拍照
                    if (!EMClient.getInstance().isConnected()) {
                        ToastUtil.showToast(getActivity(), cn.dajiahui.kid.R.string.not_connect_to_server);
                    } else {
                        selectPicFromCamera();
                    }
                    break;
    ...
    protected void sendMessage(EMMessage message) {
        ...
        message.setAttribute(UserDao.COLUMN_NAME_NICK, UserController.getInstance().getUser().getNickname());
        //发送消息
    ...
    protected void sendFileByUri(Uri uri) {
        ...
        if (file == null || !file.exists()) {
            ToastUtil.showToast(getActivity(), R.string.File_does_not_exist);
            return;
        }
        //大于10M不让发送
        if (file.length() > 10 * 1024 * 1024) {
            ToastUtil.showToast(getActivity(), R.string.The_file_is_not_greater_than_10_m);
    ...
    protected void selectPicFromCamera() {
        if (!EaseCommonUtils.isSdcardExist()) {
            ToastUtil.showToast(getActivity(), R.string.sd_card_does_not_exist, 0);
            return;
        }
    ...
    protected void toGroupDetails() {
        if (chatType == EaseConstant.CHATTYPE_GROUP) {
            EMGroup group = EMClient.getInstance().groupManager().getGroup(toChatUsername);
            if (group == null) {
                ToastUtil.showToast(getActivity(), R.string.gorup_not_found);
                return;
            }
            if (chatFragmentListener != null) {
    ...
    class GroupListener extends EaseGroupRemoveListener {
            ...
            getActivity().runOnUiThread(new Runnable() {
                ...
                public void run() {
                    if (toChatUsername.equals(groupId)) {
                        ToastUtil.showToast(getActivity(), R.string.you_are_group);
                        getActivity().finish();
                    }
                }
            });
        ...
        public void onGroupDestroy(final String groupId, String groupName) {
            ...
            getActivity().runOnUiThread(new Runnable() {
                public void run() {
                    if (toChatUsername.equals(groupId)) {
                        ToastUtil.showToast(getActivity(), R.string.the_current_group);
                        getActivity().finish();
                    }
                }
            });
